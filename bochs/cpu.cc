...
#if BX_DEBUGGER
      BX_CPU_THIS_PTR repcount = 0;
#endif
      BX_CPU_CALL_METHOD(execute, (i));
    }
    else {
#if BX_DEBUGGER
      BX_CPU_THIS_PTR repcount++;
      //BX_DEBUG(("repcount: %d", BX_CPU_THIS_PTR repcount));
#endif
...
repeat_loop:
      ...
      if (i->repeatableZFL()) {
        ...
        if (i->as64L()) {
          ...
          if (((i->repUsedValue()==3) && (get_ZF()==0)) ||
            ((i->repUsedValue()==2) && (get_ZF()!=0)) ||
            (RCX == 0)) {
#if BX_DEBUGGER
              BX_CPU_THIS_PTR repcount = 0;
#endif
